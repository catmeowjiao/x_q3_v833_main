...
static lv_obj_t* label_duration = NULL;  // 新增：录音时长显示
...
static time_t recording_start_time = 0;  // 新增：录音开始时间
static lv_timer_t* duration_timer = NULL; // 新增：时长更新定时器
...
static void start_recording(void) {
    ...
    snprintf(command, sizeof(command),
             "arecord -D hw:0,0 -f S16_LE -r 16000 '%s' &",
             current_filename);
...
static void update_space_info(void) {
    ...
    if (fp && fgets(buffer, sizeof(buffer), fp)) {
        ...
        // 格式：存储：已用/总共 MB (百分比%)
        char info[64];
        snprintf(info, sizeof(info), "存储: %.1f/%.1f MB (%d%%)", 
